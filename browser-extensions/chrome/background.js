// GoFetch Download Manager - Background Service Worker
// Intercepts downloads and sends them to local GoFetch instance
...
// === Header Capture ===
// Store request headers for URLs to forward to GoFetch (cookies, auth, etc.)
...
async function findGoFetchPort() {
  ...
  for (let port = DEFAULT_PORT; port < DEFAULT_PORT + MAX_PORT_SCAN; port++) {
    try {
      ...
      if (response.ok) {
        ...
        cachedPort = port;
        isConnected = true;
        console.log(`[GoFetch] Found server on port ${port}`);
        return port;
      }
    } catch {}
  }
...
async function checkGoFetchHealth() {
  const now = Date.now();
  ...
  const port = await findGoFetchPort();
...
async function fetchDownloadList() {
  ...
  try {
    ...
    console.error("[GoFetch] Error fetching downloads:", error);
  }
...
async function sendToGoFetch(url, filename, absolutePath) {
  const port = await findGoFetchPort();
  if (!port) {
    console.error("[GoFetch] No server found");
  ...
  try {
    ...
    if (headers) {
      ...
      console.log("[GoFetch] Forwarding captured headers to GoFetch");
    }
    ...
    if (response.ok) {
      ...
      console.log("[GoFetch] Download queued:", data);
      return { success: true, data };
    } else {
      const error = await response.text();
      console.error(
        "[GoFetch] Failed to queue download:",
        response.status,
        error,
      );
    ...
    console.error("[GoFetch] Error sending to GoFetch:", error);
...
async function pauseDownload(id) {
  ...
  try {
    ...
    console.error("[GoFetch] Error pausing download:", error);
    return false;
  }
}
...
async function resumeDownload(id) {
  ...
  try {
    ...
    console.error("[GoFetch] Error resuming download:", error);
    return false;
  }
}
...
async function cancelDownload(id) {
  ...
  try {
    ...
    console.error("[GoFetch] Error canceling download:", error);
    return false;
  }
}
...
// Check if URL is already being downloaded by GoFetch
  ...
  try {
    ...
    if (Array.isArray(list) && list.length > 0) {
      ...
      for (const dl of list) {
        ...
        // Flag as duplicate if URL exists in GoFetch's download list (any status)
        if (normalizedDlUrl === normalizedUrl) {
          console.log(
            "[GoFetch] Duplicate download detected (already in GoFetch):",
            url,
          );
          return true;
        }
    ...
    console.log("[GoFetch] Could not check GoFetch list for duplicates:", e);
  }
...
chrome.downloads.onCreated.addListener(async (downloadItem) => {
  ...
  if (!enabled) {
    console.log("[GoFetch] Interception disabled");
    return;
  }

  if (shouldSkipUrl(downloadItem.url)) {
    console.log("[GoFetch] Skipping URL type");
    return;
  }

  if (!isFreshDownload(downloadItem)) {
    console.log("[GoFetch] Ignoring historical download");
    return;
  }
...
async function handleDownloadIntercept(downloadItem) {
  // Check for duplicates (async - checks both time-based and GoFetch's download list)
  if (await isDuplicateDownload(downloadItem.url)) {
    ...
    try {
      ...
      console.log("[GoFetch] Error canceling duplicate:", e);
    }
    ...
    try {
      ...
      console.log(
        "[GoFetch] openPopup failed (might be open or no user gesture):",
        e,
      );
    }
    ...
    chrome.runtime
      ...
      .catch((e) => {
        ...
        console.log("[GoFetch] Sending promptDuplicate failed:", e);
      });
  ...
  const gofetchRunning = await checkGoFetchHealth();
  if (!gofetchRunning) {
    console.log("[GoFetch] Server not running, using browser download");
  ...
  console.log(
    "[GoFetch] Extracted path info - filename:",
    filename,
    "directory: (forced default)",
  );

  // Cancel browser download and send to GoFetch
  try {
    await chrome.downloads.cancel(downloadItem.id);
    await chrome.downloads.erase({ id: downloadItem.id });

    const result = await sendToGoFetch(downloadItem.url, filename, "");

    if (result.success) {
      if (result.data && result.data.status === "pending_approval") {
        chrome.notifications.create(`gofetch-confirm-${downloadItem.id}`, {
          type: "basic",
          iconUrl: "icons/icon48.png",
          title: "GoFetch - Confirmation Required",
      ...
      try {
        ...
        console.log("[GoFetch] Could not auto-open popup:", e.message);
      }
    } else {
      // Failed to send to GoFetch - show error notification
      chrome.notifications.create({
        type: "basic",
        iconUrl: "icons/icon48.png",
        title: "GoFetch Error",
    ...
    console.error("[GoFetch] Failed to intercept download:", error);
  }
}

// Handle notification clicks
chrome.notifications.onClicked.addListener((notificationId) => {
  if (notificationId.startsWith("gofetch-confirm-")) {
    // Attempt to open popup
    try {
      chrome.action.openPopup();
    } catch (e) {
      console.error("[GoFetch] Failed to open popup from notification:", e);
    }
    // Clear notification
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  (async () => {
    try {
      switch (message.type) {
        case "checkHealth": {
          const healthy = await checkGoFetchHealth();
        ...
        case "confirmDuplicate": {
          ...
          console.log(
            "[GoFetch] confirmDuplicate called, pending:",
          ...
          if (pending) {
            ...
            console.log(
              "[GoFetch] Sending confirmed duplicate to GoFetch:",
              pending.url,
            );
            const result = await sendToGoFetch(
              pending.url,
              pending.filename,
              pending.directory,
            );
            console.log("[GoFetch] sendToGoFetch result:", result);
        ...
        case "skipDuplicate": {
          ...
          if (pending) {
            ...
            console.log(
              "[GoFetch] User skipped duplicate download:",
              pending.url,
            );
      ...
      console.error("[GoFetch] Message handler error:", error);
...
async function initialize() {
  console.log("[GoFetch] Extension initializing...");
  await loadAuthToken();
  await checkGoFetchHealth();
  console.log("[GoFetch] Extension loaded");
}

chrome.runtime.onInstalled.addListener(() => {
  initialize().catch((error) =>
    console.error("[GoFetch] Initialization on install failed:", error),
  );
});

chrome.runtime.onStartup.addListener(() => {
  initialize().catch((error) =>
    console.error("[GoFetch] Initialization on startup failed:", error),
  );
});
