...
func LoadState(url string, destPath string) (*types.DownloadState, error) {
	...
	utils.Debug("Loading state for URL: %s, destPath: %s", url, destPath)
	...
	if err != nil {
		utils.Debug("Failed to query tasks for download %s: %v", state.ID, err)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.Offset, &t.Length); err != nil {
			utils.Debug("Failed to scan task: %v", err)
	...
	if err := rows.Err(); err != nil {
		utils.Debug("Error iterating tasks: %v", err)
		return nil, fmt.Errorf("error iterating tasks: %w", err)
	}

	utils.Debug("Successfully loaded state with %d tasks for download %s", len(state.Tasks), state.ID)
	return &state, nil
}
...
func DeleteState(id string, url string, destPath string) error {
	...
	if id != "" {
		utils.Debug("Deleting state by ID: %s", id)
		result, err = db.Exec("DELETE FROM downloads WHERE id = ?", id)
	} else {
		utils.Debug("Deleting state by URL and destPath: %s, %s", url, destPath)
		result, err = db.Exec("DELETE FROM downloads WHERE url = ? AND dest_path = ?", url, destPath)
	}

	if err != nil {
		utils.Debug("Failed to delete state: %v", err)
	...
	if rows == 0 {
		utils.Debug("No rows deleted (download already gone or didn't exist)")
		return nil
	}

	utils.Debug("Successfully deleted %d download(s)", rows)
	return nil
}
...
func LoadMasterList() (*types.MasterList, error) {
	db := getDBHelper()
	if db == nil {
		utils.Debug("DB not initialized, returning empty master list")
	...
	`)
	if err != nil {
		utils.Debug("Failed to query downloads: %v", err)
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			utils.Debug("Failed to scan download entry: %v", err)
	...
	// Check for errors from iteration
	if err := rows.Err(); err != nil {
		utils.Debug("Error iterating downloads: %v", err)
		return nil, fmt.Errorf("error iterating downloads: %w", err)
	}

	utils.Debug("Loaded master list with %d downloads", len(list.Downloads))
	return &list, nil
}
...
func AddToMasterList(entry types.DownloadEntry) error {
	...
	utils.Debug("Adding/updating download to master list: ID=%s, URL=%s", entry.ID, entry.URL)
	...
	return withTx(func(tx *sql.Tx) error {
		...
		if err != nil {
			utils.Debug("Failed to insert/update download: %v", err)
		}
		return err
	})
}
...
func RemoveFromMasterList(id string) error {
	...
	utils.Debug("Removing download from master list: %s", id)

	result, err := db.Exec("DELETE FROM downloads WHERE id = ?", id)
	if err != nil {
		utils.Debug("Failed to remove download %s: %v", id, err)
		return fmt.Errorf("failed to remove download: %w", err)
	}

	rows, _ := result.RowsAffected()
	utils.Debug("Removed download %s (rows affected: %d)", id, rows)
	return nil
}
...
func GetDownload(id string) (*types.DownloadEntry, error) {
	...
	utils.Debug("Getting download by ID: %s", id)
	...
	if err := row.Scan(
		...
		if err == sql.ErrNoRows {
			utils.Debug("Download not found: %s", id)
			return nil, nil // Not found
		}
		utils.Debug("Failed to query download %s: %v", id, err)
		return nil, fmt.Errorf("failed to query download: %w", err)
	}
...
func CheckDownloadExists(url string) (bool, error) {
	...
	if err != nil {
		utils.Debug("Failed to check download existence for URL %s: %v", url, err)
		return false, fmt.Errorf("failed to query download existence: %w", err)
	}

	utils.Debug("Download existence check for URL %s: %v (count: %d)", url, count > 0, count)
	return count > 0, nil
}
...
func UpdateStatus(id string, status string) error {
	...
	utils.Debug("Updating status for download %s to %s", id, status)

	result, err := db.Exec("UPDATE downloads SET status = ? WHERE id = ?", status, id)
	if err != nil {
		utils.Debug("Failed to update status: %v", err)
		return fmt.Errorf("failed to update status: %w", err)
	}

	rows, _ := result.RowsAffected()
	if rows == 0 {
		utils.Debug("Download not found for status update: %s", id)
...
func RemoveCompletedDownloads() (int64, error) {
	...
	utils.Debug("Removing all completed downloads")

	result, err := db.Exec("DELETE FROM downloads WHERE status = 'completed'")
	if err != nil {
		utils.Debug("Failed to remove completed downloads: %v", err)
		return 0, fmt.Errorf("failed to remove completed downloads: %w", err)
	}

	count, _ := result.RowsAffected()
	utils.Debug("Removed %d completed downloads", count)
	return count, nil
}
...
// ValidateIntegrity checks that paused .GoFetch files still exist and haven't been tampered with.
...
func ValidateIntegrity() (int, error) {
	...
	for _, e := range entries {
		GoFetchPath := e.destPath + types.IncompleteSuffix

		// Check if .GoFetch file exists
		if _, err := os.Stat(GoFetchPath); os.IsNotExist(err) {
			// File missing — remove orphaned DB entry
			utils.Debug("Integrity: .GoFetch file missing for %s, removing entry %s", e.destPath, e.id)
		...
		if e.fileHash != "" {
			currentHash, err := computeFileHash(GoFetchPath)
			if err != nil {
				utils.Debug("Integrity: failed to hash %s: %v", GoFetchPath, err)
			...
			if currentHash != e.fileHash {
				...
				utils.Debug("Integrity: hash mismatch for %s (expected %s, got %s), removing", GoFetchPath, e.fileHash, currentHash)
				_ = os.Remove(GoFetchPath)
				_ = RemoveFromMasterList(e.id)
				_, _ = db.Exec("DELETE FROM tasks WHERE download_id = ?", e.id)
				removed++
			}
